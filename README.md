...
[![PyPI](https://img.shields.io/pypi/v/cognis-oraclewatch.svg)](https://pypi.org/project/cognis-oraclewatch/)
[![CI](https://github.com/cognis-digital/oraclewatch/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/oraclewatch/actions)
...
ORACLEWATCH exists for one job — monitors price-oracle feeds for staleness, deviation, and manipulation exposure, simulating twap/spot attack profitability per pool. — and does it without a SaaS bill or heavyweight setup.
...
```bash
oraclewatch --version
oraclewatch scan .                      # scan the current project
oraclewatch scan . --format json
oraclewatch scan . --fail-on high       # non-zero exit for CI gates
oraclewatch mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Chainlink monitoring / Tellor watchdogs**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`oraclewatch` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
